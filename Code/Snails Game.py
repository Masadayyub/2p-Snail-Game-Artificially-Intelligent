...
class MainView(arcade.View):
    ...
    def on_draw(self):
        arcade.start_render()
        textture1=arcade.load_texture("F:/ai projects/Snail game/mainBack.jpg")
        scaled1=1
        ...
        textture5=arcade.load_texture("F:/ai projects/Snail game/icons.png")
        scaled5=1
...
class InstructionView(arcade.View):
    ...
    def on_draw(self):
        arcade.start_render()
        textture1=arcade.load_texture("F:/ai projects/Snail game/instruction.jpg")
        scaled1=1.05
        arcade.draw_scaled_texture_rectangle(685, 385, textture1, scaled1, 0)
    def on_mouse_press(self, _x, _y, _button, _modifiers):
        game_view = GameView()
...
class GameView(arcade.View):
    def __init__(self):
        super().__init__()
        self.tmp_counter = (40,40) #stores location of player 1's sprite
        self.tmp_counter2 = (660,660)   #stores location of AI-Bot's sprite
        self.grid = [] 
        self.a1 = 0 
        self.stuck1 = 0 
        self.stuck2 = 0
        ...
        #stores data of both sprites (initialize sprites)
        self.grid[0][0] = 1
        self.grid[9][9] = 2
    ...
    def on_draw(self): #drawing the background, sprites , game view screen
        arcade.start_render()
        textture0=arcade.load_texture("F:/ai projects/Snail game/bck.jpg")
        scaled0=.6
        arcade.draw_scaled_texture_rectangle(700, 350, textture0, scaled0, 0)
        textture4=arcade.load_texture("F:/ai projects/Snail game/PL1.png")
        scaled4=.3
        arcade.draw_scaled_texture_rectangle(310, 700, textture4, scaled4, 0)

        textture6=arcade.load_texture("F:/ai projects/Snail game/AD.png")
        scaled6=.2
        arcade.draw_scaled_texture_rectangle(1100, 600, textture6, scaled6, 0)
        
        a=arcade.load_texture("F:/ai projects/Snail game/score.png")
        b=.3
        ...
        for row in range(ROW_COUNT):  #iterate on ecery grid position
            for column in range(COLUMN_COUNT):
                ...
                if self.grid[row][column] == 1: # place player 1's sprite on board
                    p1 = arcade.load_texture("F:/ai projects/Snail game/SNAIL1.png")
                    size = 0.03
                    arcade.draw_scaled_texture_rectangle(x,y,p1,size,0)
                    

                if self.grid[row][column] == 2: #place AI-bot's sprite on board
                    p2 = arcade.load_texture("F:/ai projects/Snail game/SNAIL2.png")
                    size = 0.025
                    arcade.draw_scaled_texture_rectangle(x,y,p2,size,0)
                
                if self.grid[row][column] == 10: #place splash of player 1
                    sp1 = arcade.load_texture("F:/ai projects/Snail game/sp1.png")
                    size = 0.025
                    arcade.draw_scaled_texture_rectangle(x,y,sp1,size,0)
                
                if self.grid[row][column] == 20: #place splash of AI-bot
                    sp2 = arcade.load_texture("F:/ai projects/Snail game/sp2.png")
                    size = 0.03
                ...
                #draw board (front-end) 
                arcade.draw_rectangle_outline(x,y,WIDTH,HEIGHT,arcade.color.WHITE,2)
    
    
    def legalmove(self,row,column,pre_row,pre_col): #checks whether move is legal or not
        if row < ROW_COUNT and column < COLUMN_COUNT : #whether clicked inside or outside grid 
    ...
    def heuristic(self,grid):#computes the winning chances of Ai-Bot using minimax
        winningChances = 0
    # First Condition 
        count=0
    ...
    def splashed1(self,row,column,p_row,p_column):
          #This function is called when player 1 clicks on its own splash, makes it slippery
#checks all the possible conditions, anyone of these will be applicable
        ...
        return row,column #returns the desired row and column where the sprite should go after slip

    def splashed2(self,row,column,p_row,p_column):
        #This function is called when AI-BOt clicks on its own splash, makes it slippery
#checks all the possible conditions, anyone of these will be applicable
        if row == p_row and column == p_column-1:
            for a in range(0,9):
                if column >=0 :
    ...
    def on_mouse_press(self, x, y, button, modifiers):  #when mouse is pressed 
        
    #Evaluates the board on every click to check whether any of the player won or game is drawn
        ...
        #gets the backend row and coloumn after calculating board's co-ordinates
        ...
        if self.legalmove(row,column,p_row,p_col) == 1:
            if self.grid[row][column] == 0: #if legal then increases the score and updates the board
                self.grid[row][column] = 1
                self.grid[p_row][p_col] = 10
                n_row = (MARGIN + WIDTH) * row + MARGIN + WIDTH // 2
                n_col = (MARGIN + HEIGHT) * column + MARGIN + HEIGHT // 2
                self.tmp_counter = (n_row,n_col) #stores these co-ordinates for the next click, these will be used as prev, row and coloumns
                self.window.score1=self.window.score1+1
                self.stuck1 = 0
#if player clicked on its own splash, then slippery surface should be called
...
#========================================================[ AI BOT PLAYING  ]===================================================================
        ...
        pre_coord = self.position(self.grid,2) #current position of AI-Bot
        p_row = pre_coord[0]
        p_col = pre_coord[1]

        co_ordinates =  self.findbestmove(self.grid) #Finds the best move using Bestmove function
        row = co_ordinates[0]
        col = co_ordinates[1]
        ...
        elif self.grid[row][col] == 20: #slippery_surface
...
class GameOverView(arcade.View):  #Called when game is over
